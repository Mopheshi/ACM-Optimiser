    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                # Tangent Space Inertia & Directional Density
                momentum.mul_(beta1).add_(grad, alpha=1.0 - beta1)
                density.mul_(beta2).addcmul_(grad, grad, value=1.0 - beta2)

                # Momentum Bias Correction (Satisfies Theorem 1 Assumption)
                bias_correction1 = 1.0 - beta1 ** step
                m_hat = momentum / bias_correction1

                # The ACM Metric Tensor (Raw Density - Satisfies Lemma 1 Bounds)
